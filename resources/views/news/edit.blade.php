<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('news.update', $news->NewsId) }}" method="POST" class="space-y-6" id="newsEditForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="Category" id="Category"
                            ...
                            <option value="Deportes"        {{ old('Category', $news->Category) == 'Deportes'        ? 'selected' : '' }}>Deportes</option>
                            <option value="Tecnología"      {{ old('Category', $news->Category) == 'Tecnología'      ? 'selected' : '' }}>Tecnología</option>
                            <option value="Negocios"        {{ old('Category', $news->Category) == 'Negocios'        ? 'selected' : '' }}>Negocios</option>
                            <option value="Entretenimiento" {{ old('Category', $news->Category) == 'Entretenimiento' ? 'selected' : '' }}>Entretenimiento</option>
                        </select>
                    ...
                    <div>
                        ...
                        @if($news->ImageUrl)
                            <div class="mb-4" id="currentImageWrapper">
                                <p class="text-sm text-gray-600 mb-2">Imagen actual:</p>
                                <div class="relative inline-block">
                                    <img src="{{ $news->ImageUrl }}" alt="Imagen actual"
                                         class="max-w-full h-auto max-h-64 rounded-lg border-2 border-gray-200" id="currentImage">
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="imageFile" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500">
                                        <span>{{ $news->ImageUrl ? 'Cambiar imagen' : 'Subir una imagen' }}</span>
                        ...
                        @error('ImageUrl') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        <div id="imagePreview" class="mt-4 hidden">
                            <p class="text-sm font-medium text-gray-700 mb-2">Nueva imagen:</p>
                            <div class="relative inline-block">
                                <img id="previewImage" src="" alt="Preview" class="max-w-full h-auto max-h-96 rounded-lg border-2 border-green-200">
                                <button type="button" id="removeNewImage" class="absolute top-2 right-2 bg-red-600 text-white rounded-full p-2 hover:bg-red-700 shadow-lg">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                    </svg>
                                </button>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="url" name="InstagramUrl" id="InstagramUrl"
                                       value="{{ old('InstagramUrl', $news->InstagramUrl) }}"
                                ...
                                <p class="mt-1 text-xs text-gray-500">📸 Posts como embed. 🎬 Reels como enlace directo.</p>
                                @error('InstagramUrl') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="url" name="SpotifyUrl" id="SpotifyUrl"
                                       value="{{ old('SpotifyUrl', $news->SpotifyUrl) }}"
                                       placeholder="https://open.spotify.com/track/XXXXXXX"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                <p class="mt-1 text-xs text-gray-500">🎵 Track, álbum, playlist o episodio.</p>
                                @error('SpotifyUrl') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="url" name="FacebookUrl" id="FacebookUrl"
                                       value="{{ old('FacebookUrl', $news->FacebookUrl) }}"
                                       placeholder="https://www.facebook.com/watch/?v=XXXXXXX"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                <p class="mt-1 text-xs text-gray-500">📹 Posts y videos públicos de Facebook.</p>
                                @error('FacebookUrl') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                    ...
                    <div class="flex items-center space-x-3">
                        <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-md shadow hover:bg-indigo-700">
    ...
    <script>
        ...
        const removeNewButton  = document.getElementById('removeNewImage');
